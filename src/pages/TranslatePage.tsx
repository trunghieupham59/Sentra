...
export function TranslatePage() {
  ...
  return (
    <div className="app-page">
      <div className="app-workspace" style={{ paddingTop: 16 }}>
        <div className="app-panel-grid">

          {/* ═══ SOURCE PANEL ═══ */}
          <section
            aria-label={t.image_translate_title}
            className={`glass-panel flex flex-col h-full relative transition-colors duration-150 ${
          ...
          >
            {/* Panel header: source language selector + AI config gear */}
            <div className="panel-header">
              ...
              <div className="relative flex-shrink-0" ref={aiConfigRef}>
                <button
                  ...
                  className={`btn-icon cursor-pointer ${showAIConfig ? 'btn-icon-active' : ''}`}
                >
                  <GearIcon className="w-3.5 h-3.5" />
                </button>

                {showAIConfig && (
                  <div className="floating-panel ai-config-panel absolute top-full left-0 mt-2 z-50 w-[520px] p-4 flex flex-col gap-4">
                    <h2 className="popover-title">{t.translate_ai_config_title}</h2>
                    <ModelSelector />
                    <div style={{ borderTop: '1px solid var(--vzn-divider)' }} />
                    <TranslateToolbar
                      hideModelSelector
            ...
            {imageAttachment && !isVoiceActive && (
              <ImageAttachmentPreview imageAttachment={imageAttachment} onRemove={handleRemoveImage} />
            )}

            {/* Text area */}
            <div ref={sourceScrollRef} className="flex-1 min-h-0 overflow-auto p-4 flex flex-col">
              <textarea
                value={sourceText}
            ...
            <div className="surface-footer relative z-20">
              ...
              />
              <input ref={fileInputRef} type="file" accept={ACCEPTED_IMAGE_MIME_TYPES} className="hidden" onChange={handleFileInputChange} />
              <div className="flex items-center gap-3">
                {!isVoiceActive && (
                  charCount > MAX_INPUT_CHARS ? (
                    <span className="flex items-center gap-1 text-xs tabular-nums font-medium" style={{ color: 'var(--vzn-danger)' }} title={t.translate_limit}>
                      <AlertTriangleIcon className="w-3 h-3 flex-shrink-0 text-red-400" />
          ...
          <div className="glass-panel flex flex-col h-full">
            {/* Panel header: swap button + target language selector */}
            <div className="panel-header">
              <button
                type="button"
                ...
                style={{ background: 'transparent', border: 'none', color: canSwap ? 'var(--vzn-accent)' : 'var(--vzn-text-disabled)' }}
              >
              ...
              {isTranslating ? (
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="flex flex-col items-center gap-3">
                    <SpinnerIcon className="w-5 h-5 animate-spin text-indigo-400" />
                ...
                <img src={editedImageUrl} alt={t.image_translate_title} className="max-w-full rounded-lg fade-in" />
              ) : translatedText ? (
                ...
                <p className="ui-reader-muted select-none">{t.translate_result_placeholder}</p>
              )}
            </div>
